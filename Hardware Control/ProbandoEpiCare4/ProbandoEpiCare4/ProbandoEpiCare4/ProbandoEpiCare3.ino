...
// Constantes para detectar caídas del MPU 
const int umbralAngulo = 30; 

// Funciones del MPU6050 
void leerAcelerometro(float *accX, float *accY, float *accZ); 
float calcularAceleracionAngular(float accX, float accY, float accZ); 
bool detectarCaida(float aceleracionAngular, float umbralAngulo); 
...
void setup() {
  ...
  leerAcelerometro = &leerAcelerometro;
  calcularAceleracionAngular = &calcularAceleracionAngular;
  detectarCaida = &detectarCaida;
...
void loop() 
{
  ...
  leerAcelerometro(&accX, &accY, &accZ);

  // Calcula la aceleración angular
  float aceleracionAngular = calcularAceleracionAngular(accX, accY, accZ);

  // Detecta una caída
  bool caidaDetectada = detectarCaida(aceleracionAngular, umbralAngulo);

  // Envía una señal de alarma si se detecta una caída
  if (caidaDetectada) 
  {
